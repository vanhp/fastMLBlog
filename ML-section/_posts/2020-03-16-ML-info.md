...
Pandas a Python library for database, it's fast and flexible library but has non-intuative interface. It's used to stored manage analize structure data type. It's also useful in ML to handle tabular and timeseries data type.
...
it has facility to handle Datasets
which auto split data into train 80% validate 20%
...
The loss function that suitable for case where there more than one label or no label present
