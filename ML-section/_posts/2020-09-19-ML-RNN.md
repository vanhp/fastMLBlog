...
Internally LSTM comprise with little neural net that decise how much gradient is needed and which one to update or delete. The little NN is compose of input x hidden state ht-1 and previous state Ct-1 into current node then out via sigmoid ($$\sigma$$) or tanh  a sigmoid function rescaled to the range -1 to 1 to the next
