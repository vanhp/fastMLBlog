...
PyTorch provide nll_loss assumes that you already took the log of the softmax, so it doesn't actually do the logarithm must use log_softmax before nll_loss
...
- Confusion matrix show grid of predict vs actual value
  - most_confused (interp.most_confused(min_val=5)) to show model confusion the diagonal line is correct a 1 is any place else show mis-prediction
